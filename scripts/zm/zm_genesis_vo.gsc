...
/*
	...
	Flags: Linked
*/
function vo_say(str_vo_alias, n_delay = 0, b_wait_if_busy = 0, n_priority = 0, var_d1295208 = 0, var_248b6239 = 0, var_a5d9e5f7 = undefined)
{
	self endon("death");
...
function function_2426269b(v_pos, n_range = 1000)
{
	...
	{
		foreach(e_spkr in level.a_e_speakers)
		{
			if(!isdefined(e_spkr))
			{
				continue;
			}
			if(!isdefined(v_pos) || distancesquared(e_spkr.origin, v_pos) <= (n_range * n_range))
			{
				if(isdefined(e_spkr.str_vo_being_spoken) && e_spkr.str_vo_being_spoken != "")
				{
					e_spkr stopsound(e_spkr.str_vo_being_spoken);
				}
				e_spkr.deleteme = 1;
				e_spkr.str_vo_being_spoken = "";
				e_spkr.n_vo_priority = 0;
				e_spkr.isspeaking = 0;
			}
		}
		i = 0;
...
function function_63c44c5a(var_cbd11028, var_e21e86b8, b_wait_if_busy = 0, n_priority = 0, var_d1295208 = 0, var_248b6239 = 0)
{
	...
	for(i = 0; i < var_cbd11028.size; i++)
	{
		...
		self vo_say(var_cbd11028[i], var_e27770b1, b_wait_if_busy, n_priority, var_d1295208, var_248b6239);
	}
	function_218256bd(0);
}
...
function function_dfe962f()
{
	if(!(isdefined(level.var_4a394cdf) && level.var_4a394cdf))
	{
		...
		if(level.activeplayers.size == 1)
		{
			n_ndx = level.activeplayers[0].characterindex;
		}
		else
		{
			e_richtofen = zm_utility::get_specific_character(2);
			e_closest_player = zm_utility::get_player_closest_to(e_richtofen);
			n_ndx = e_closest_player.characterindex;
		}
		level waittill("audio_log_complete");
		if(isdefined(level.var_8c92b387["sophia_appears_1_convo"][n_ndx]))
		{
			var_d44b84c3 = level.var_8c92b387["sophia_appears_1_convo"][n_ndx];
			function_7aa5324a(var_d44b84c3, 1);
			level.var_8c92b387["sophia_appears_1_convo"][n_ndx] = undefined;
		}
	}
}
...
function function_efdd99e2(e_player)
{
	if(!(isdefined(level.var_d831dda4) && level.var_d831dda4))
	{
		...
		n_ndx = e_player.characterindex;
		var_d44b84c3 = level.var_8c92b387["sophia_appears_2_convo"][n_ndx];
		function_7aa5324a(var_d44b84c3, 1);
...
function function_60fe98c4()
{
	...
	var_d44b84c3 = level.var_8c92b387[var_bd381e73][n_ndx];
	if(isdefined(var_d44b84c3))
	{
		function_7aa5324a(var_d44b84c3);
		level.var_8c92b387[var_bd381e73][n_ndx] = undefined;
	}
}
...
{
	n_ndx = e_player.characterindex;
	if(isdefined(level.var_8c92b387["summoning_key_pickup"][n_ndx]))
	{
		function_897246e4(level.var_8c92b387["summoning_key_pickup"][n_ndx]);
		level.var_8c92b387["summoning_key_pickup"][n_ndx] = undefined;
...
function function_b28d80bd(var_9485d9b7)
{
	/#
		...
		{
			e_character vo_say("", 0, 0, 1000, 1, 1);
		}
	#/
}
