...
function function_27b96bc()
{
	...
	while(level.var_6e907685.size)
	{
		...
		if(isdefined(e_ball.carrier) || (isdefined(e_ball.isresetting) && e_ball.isresetting))
		{
			util::wait_network_frame();
			continue;
		}
			...
			foreach(s_toy in level.var_6e907685)
			{
				...
				if(n_length < s_toy.radius)
				{
					...
					switch(s_toy.script_string)
					{
						...
						case "forward":
						{
							var_39815a1b = (vectornormalize(e_ball.lastcarrier.origin - s_toy.origin)) * 160;
							break;
						}
						default:
						{
...
/*
	...
	Flags: Linked
*/
function function_64e4c3ee(b_lock = 1)
{
	foreach(e_player in level.activeplayers)
	{
		if(b_lock)
