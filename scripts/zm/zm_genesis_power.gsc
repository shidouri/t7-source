...
function function_79774b04()
{
	...
	while(true)
	{
		...
		for(i = 0; i < a_players.size; i++)
		{
			...
			{
				e_pad = a_pads[j];
				if(!(isdefined(e_pad.var_56e27fd0) && e_pad.var_56e27fd0))
				{
					var_cebeadd5 = e_pad function_8961cbdb(e_player);
					if(var_cebeadd5)
					{
						var_cebeadd5 = 1;
						break;
			...
			if(isdefined(e_player.var_e3e6c76a))
			{
				n_cost = function_fe2cf77b(e_pad);
...
/*
	...
	Flags: Linked
*/
function function_fe2cf77b(e_pad)
{
	return 500;
}
...
/*
	...
	Flags: Linked
*/
function function_b8c60cae(e_pad)
{
	n_num = 0;
	a_players = getplayers();
	for(i = 0; i < a_players.size; i++)
	{
		e_player = a_players[i];
		if(function_d46b0523(e_player, e_pad))
		{
			n_num++;
		}
	}
...
/*
	...
	Flags: Linked
*/
function function_d46b0523(e_player, e_pad)
{
	if(e_player istouching(e_pad) && (!(isdefined(e_player.laststand) && e_player.laststand)))
	{
		return true;
	}
	return false;
...
/*
	...
	Flags: Linked
*/
function function_59bcf901(n_cost, e_pad)
{
	self endon("death");
	...
	str_struct = "power_pickup" + e_pad.script_int;
	s_struct = struct::get(str_struct, "targetname");
	wait(1.5);
...
function function_6301de9e(v_pos, v_angles)
{
	...
	if(isplayer(e_attacker))
	{
		...
		{
			e_pad = level.var_7b91fc17;
			if(e_pad.var_3f80e7e8 < 200)
			{
				e_pad.var_3f80e7e8 = e_pad.var_3f80e7e8 + 10;
...
{
	e_fury = zm_genesis_apothicon_fury::function_21bbe70d(s_spawn_pos.origin, s_spawn_pos.angles, 0);
	e_fury ai::set_behavior_attribute("can_bamf", 0);
	e_fury ai::set_behavior_attribute("can_juke", 0);
	e_fury.no_damage_points = 1;
	e_fury.deathpoints_already_given = 1;
	if(isdefined(e_fury))
	{
		e_fury endon("death");
		e_fury function_da370997(var_d24a63df);
		wait(0.5);
		...
		if(level.var_eada0345 == 0)
		{
			if(math::cointoss())
			{
				e_fury ai::set_behavior_attribute("move_speed", "walk");
			}
			else
			{
