...
function init()
{
	...
	for(i = 0; i < arenas.size; i++)
	{
		...
		{
			camang = strtok(var_5fabae4f.script_noteworthy, " ");
			/#
				assert(isdefined(camang.size == 3), ("" + arena.name) + "");
			#/
			arena.var_5fec1234 = (float(camang[0]), float(camang[1]), float(camang[2]));
		}
		else
		{
		...
		if(isdefined(var_5fabae4f))
		{
			...
			if(isdefined(var_5fabae4f.script_noteworthy))
			{
				...
				/#
					assert(isdefined(camang.size == 3), ("" + arena.name) + "");
				#/
				arena.var_a8b67ea4 = (float(camang[0]), float(camang[1]), float(camang[2]));
			}
			else
			{
...
function restart()
{
	...
	level.doa.var_1a75d02b = 1;
	level.doa.flipped = 0;
	doa_camera::function_d22ceb57(vectorscale((1, 0, 0), 75), function_61e2e743());
...
function flipcamera(localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump)
{
	...
	switch(level.doa.var_1a75d02b)
	{
		...
		{
			level.doa.flipped = 0;
			doa_camera::function_d22ceb57(function_c530afc5(vectorscale((1, 0, 0), 75)), function_61e2e743());
			if(isdefined(level.doa.arenas[level.doa.current_arena].var_2ac7f133))
			{
				level.doa.arenas[level.doa.current_arena].var_7526f3f5 = level.doa.arenas[level.doa.current_arena].var_2ac7f133;
			}
