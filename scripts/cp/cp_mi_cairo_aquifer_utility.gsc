	...
	while(isdefined(self) && isalive(self))
	{
		...
		if(isdefined(self.dogfighter) && attacker == self.dogfighter)
		{
			...
			if(self.var_d4f48128 >= damage_stages)
			{
				...
				{
					self.dogfighter aquifer_accolades::accolade_increment("aq_dogfight_kill_only_guns");
				}
				return;
			}
			if(self.var_d4f48128 > 1)
...
function function_9476c2d5()
{
	...
	while(true)
	{
		...
		{
			ideal_pitch = angleclamp180(vectortoangles(self.var_bded8100.origin - self.origin)[0]);
			ideal_pitch = math::clamp(ideal_pitch, -30, 30);
		}
		if((abs(ideal_pitch - self.var_51cc2ae)) <= self.var_36c3df0c)
		{
			self.var_51cc2ae = ideal_pitch;
		}
		else
		{
			if(ideal_pitch < self.var_51cc2ae)
			{
